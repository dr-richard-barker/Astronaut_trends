...
# --- Fig 6: orbital vs suborbital over time ---
def fig_orbital_suborbital():
    if "FlightClass" not in usable.columns:
        print("  (skip fig6: FlightClass not present — rebuild dataset)")
        return
    piv = (usable.groupby(["FirstMissionYear", "FlightClass"]).size()
           .unstack(fill_value=0))
    for col in ("Orbital", "Suborbital"):
        if col not in piv:
            piv[col] = 0
    piv = piv.sort_index()
    fig, ax = plt.subplots(figsize=(10, 3.8))
    ax.bar(piv.index, piv["Orbital"], color="#2a78d6", label="Orbital (reached orbit)")
    ax.bar(piv.index, piv["Suborbital"], bottom=piv["Orbital"], color="#eb6834",
           label="Suborbital only")
    ax.set_ylabel("New astronauts")
    ax.set_xlabel("Year of first spaceflight")
    ax.set_title("Orbital vs suborbital spaceflight over time", fontweight="bold")
    ax.legend(frameon=False, loc="upper left")
    save(fig, "fig6_orbital_suborbital.png")
...
def markdown_tables():
    lines = ["# Astronaut Trends — Summary Tables",
             ...
             ""]
    n = len(usable)
    orb = int((usable.get("FlightClass") == "Orbital").sum()) if "FlightClass" in usable else 0
    sub = int((usable.get("FlightClass") == "Suborbital").sum()) if "FlightClass" in usable else 0
    lines += ["## Headline figures", "",
              ...
              f"- **Reached orbit:** {orb} ({100*orb/n:.1f}%) · **suborbital only:** {sub} ({100*sub/n:.1f}%)",
    ...
    # Table: flight class by country
    if "FlightClass" in usable:
        lines += ["## Orbital vs suborbital by country", "",
                  "| Country | Orbital | Suborbital only | % suborbital |",
                  "|---|--:|--:|--:|"]
        for c in COUNTRIES:
            sc = usable[usable.CitizenGroup == c]
            o = int((sc.FlightClass == "Orbital").sum())
            s = int((sc.FlightClass == "Suborbital").sum())
            tot = max(1, o + s)
            lines.append(f"| {c} | {o} | {s} | {100*s/tot:.1f}% |")
        lines.append("")
